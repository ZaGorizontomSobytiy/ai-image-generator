#!/usr/bin/env python3
"""
CLI приложение для генерации изображений через ProxyAPI (gpt-image-1)
...
def get_proxy_api_key():
    ...
    if not api_key:
        ...
        print("Добавьте ключ в файл .env: PROXY_API=ваш_ключ")
        sys.exit(1)
    return api_key
...
def create_output_directory():
    ...
    output_dir = Path("generated_images/proxyapi")
    output_dir.mkdir(parents=True, exist_ok=True)
    return output_dir


def generate_image(client, prompt, output_dir):
    """Генерирует изображение по промпту через ProxyAPI и сохраняет его"""
...
def main():
    ...
    print("=" * 60)
    print("Генератор изображений через ProxyAPI (gpt-image-1)")
    print("=" * 60)
